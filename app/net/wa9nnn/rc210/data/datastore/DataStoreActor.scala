...
object DataStoreActor extends ActorModule with NamedSource with LazyLogging {
  ...
  case class TriggersForMacro(macroKey: MacroKey, replyTo: ActorRef[Seq[FieldEntry]]) extends DataStoreMessage

  case class AcceptCandidate(fieldKey: FieldKey, user: User) extends DataStoreMessage


  case class UpdateData(candidates: Seq[UpdateCandidate], namedKeys: Seq[NamedKey] = Seq.empty, user: User, replyTo: ActorRef[Seq[FieldEntry]]) extends DataStoreMessage
    ...
    Behaviors.setup[DataStoreMessage] { actorContext =>
      ...
      Behaviors.receiveMessage { message =>
        message match {
          ...
          case TriggersForMacro(macroKey: MacroKey, replyTo: ActorRef[Seq[FieldEntry]]) =>
            replyTo ! all.flatMap(_.canTriggerMacro(macroKey)).sortBy(_.fieldKey)
